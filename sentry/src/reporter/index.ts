...
import { CallbackQueue, sentry, larkLogger } from "../utils";
...
export class DataReporter implements IDataReporter {
  ...
  private initNetworkListener() {
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("offline", () => {
        this.isOnline = false;
        larkLogger.info("lark Sentry", "Network is offline, caching events");
      });
    }
  }
  ...
  reportByFetch(data: any) {
    const cb = () => {
      fetch(sentry.options.dsn, {
        ...
        .catch((err) => {
          larkLogger.error("lark Sentry Report", "Fetch report failed", err);
          this.handleServerError();
        });
    };
  ...
  async send(payload: TReportPayload, immediate = false) {
    ...
    larkLogger.info("lark Sentry Report", `Type: ${payload.type}`, data);
